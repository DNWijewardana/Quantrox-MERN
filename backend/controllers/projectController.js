...
export const listProjects = async (req, res) => {
  try {
    ...
    const projects = await projectModel
      .find({ userId })
      .select(
        "name description status planFile estimate.totalCost createdAt updatedAt",
      )
      .sort({ createdAt: -1 });
...
export const updateScale = async (req, res) => {
  try {
    ...
    if (pixelsPerMeter !== undefined) {
      ...
      project.scale.metersPerPixel =
        pixelsPerMeter > 0 ? 1 / pixelsPerMeter : 0;
    }
    ...
    if (referenceLength !== undefined)
      project.scale.referenceLength = referenceLength;
    if (referencePixels !== undefined)
      project.scale.referencePixels = referencePixels;
